...
namespace triple::core {
	...
	Engine::Engine()
	    : impl(new Impl()), m_isRunning(false), m_isInitialized(false), m_lastTime(0.0f) {
	...
	bool Engine::init() {
		...
		impl->renderSystem =
		    registerSystem(std::make_unique<RenderSystem>(impl->assetsService), impl);
		impl->inputSystem = registerSystem(std::make_unique<InputSystem>(), impl);
		impl->inputActionSystem =
		    registerSystem(std::make_unique<InputActionSystem>(impl->inputSystem), impl);
	...
	Engine::ErrorCode Engine::run(const char *title, unsigned int width, unsigned int height) {
		...
		OpenGLRenderModule *openGLModule = dynamic_cast<OpenGLRenderModule *>(
		    impl->moduleService->getModule(ModuleType::OpenGLRenderer));
	...
	void Engine::onRender(float t) {
		...
		if (!transform || !cameraComp) {
			triple::log::Logger::ModuleWarn(this->getModuleName(),
			                                "Active camera missing on render");
			return;
		}
	...
	Scene *Engine::getActiveScene() const { return impl->scene.get(); }

	IWindow *Engine::getWindow() const { return impl->window.get(); }

	void Engine::setActiveCamera(Entity camera) const { impl->cameraEntity = camera; }
	...
	void Engine::loadSystemCallbacks() {
		...
		impl->eventService->addListener<KeyboardInputEvent>(
		    [this](KeyboardInputEvent &e) { impl->inputSystem->onKeyboard(e); });

		impl->eventService->addListener<MouseMoveEvent>(
		    [this](MouseMoveEvent &e) { impl->inputSystem->onMouseMove(e); });

		impl->eventService->addListener<MouseButtonEvent>(
		    [this](MouseButtonEvent &e) { impl->inputSystem->onMouseButton(e); });
	}

	void Engine::loadAssetsCallbacks() {
		impl->assetsService->setTextureLoadedCallback(
		    [this](const Texture *tex) { impl->renderSystem->uploadTexture(tex); });

		impl->assetsService->setModelLoadedCallback(
		    [this](const Model *model) { impl->renderSystem->uploadGeometry(model); });

		impl->assetsService->setShaderLoadedCallback(
		    [this](const Shader *shader) { impl->renderSystem->uploadShader(shader); });
	}
	...
	bool Engine::bootstrapResources() const { return impl->assetsService->loadDefaultAssets(); }
