...
namespace triple::gl {
	...
	bool GLShader::compileProgram(const std::string &verSource, const std::string &fragSource) {
		if (!this->isCompiled()) {
			unsigned int vertexShader =
			    compileShader(verSource, GL_VERTEX_SHADER, this->m_errorLog);
			if (vertexShader == 0) {
				return false;
			}
			unsigned int fragmentShader =
			    compileShader(fragSource, GL_FRAGMENT_SHADER, this->m_errorLog);
			if (fragmentShader == 0) {
	...
	unsigned int GLShader::linkProgram(unsigned int vertexShader, unsigned int fragmentShader,
	                                   std::string &outLog) {
	...
	bool GLShader::operator==(const GLShader &other) const {
		return m_programID == other.m_programID && m_compiled == other.m_compiled &&
		       m_errorLog == other.m_errorLog;
	}
	bool GLShader::operator!=(const GLShader &other) const { return !(*this == other); }
	GLShader::GLShader(GLShader &&other) noexcept
	    : m_programID(other.m_programID), m_compiled(other.m_compiled),
