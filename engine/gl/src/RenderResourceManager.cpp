...
namespace triple::gl
{
	...
	gfx::GPUHandle RenderResourceManager::createGLGeometry(const gfx::GeometryDesc& desc) {
		...
		geom.vbo.setData(desc.vertices, desc.vertexCount * sizeof(gfx::Vertex), VertexBufferObject::Usage::STATIC_DRAW);
		geom.ibo.setData(desc.indices, desc.indexCount * sizeof(uint32_t), IndexBufferObject::Usage::STATIC_DRAW);
	...
	gfx::GPUHandle RenderResourceManager::createGLShader(const gfx::ShaderDesc& desc) {
		...
		{
			triple::log::Logger::ModuleCritical("OpenGL -> RenderResourceManager", "Failed to compile shader program: " + shader.getErrorLog());
