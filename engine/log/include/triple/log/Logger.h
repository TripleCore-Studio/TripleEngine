...
namespace triple::log {
    ...
    public:
        ...
        template<typename... Args>
        static void ModuleInfo(std::string_view moduleName, std::string_view fmt_str, Args&&... args) {
            LogMessage(LogLevel::Info, fmt::format("[{}] -> {}", moduleName, 
                fmt::vformat(fmt_str, fmt::make_format_args(args...))));
        }

        template<typename... Args>
        static void ModuleWarn(std::string_view moduleName, std::string_view fmt_str, Args&&... args) {
            LogMessage(LogLevel::Warn, fmt::format("[{}] -> {}", moduleName, 
                fmt::vformat(fmt_str, fmt::make_format_args(args...))));
        }

        template<typename... Args>
        static void ModuleError(std::string_view moduleName, std::string_view fmt_str, Args&&... args) {
            LogMessage(LogLevel::Error, fmt::format("[{}] -> {}", moduleName, 
                fmt::vformat(fmt_str, fmt::make_format_args(args...))));
        }

        template<typename... Args>
        static void ModuleCritical(std::string_view moduleName, std::string_view fmt_str, Args&&... args) {
            LogMessage(LogLevel::Critical, fmt::format("[{}] -> {}", moduleName, 
                fmt::vformat(fmt_str, fmt::make_format_args(args...))));
        }
