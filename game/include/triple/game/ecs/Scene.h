...
namespace triple::game {
	...
	struct ComponentView {
		...
		T &operator[](size_t index) const { return *reinterpret_cast<T *>(data + index * stride); }
	};
	...
	public:
		...
		ComponentView<T> getView() {
			...
			return {raw.entities, raw.data, raw.count, raw.stride};
		}
		...
		T *getComponentByIndex(size_t index, Entity fallbackEntity) {
			...
			if (id == INVALID_COMPONENT_TYPE_ID)
				return nullptr;
			T *component = static_cast<T *>(getComponentByIndexChecked(index, id, fallbackEntity));
			if (!component) {
				return getComponent<T>(fallbackEntity);
			}
			return component;
